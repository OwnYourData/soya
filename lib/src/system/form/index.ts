...
class FormBuilder {
  ...
  private _handleClass = async (
    ...
    SELECT * WHERE {
      ...
      OPTIONAL {
        ?prop soya:container_type ?containerType .
      }
    ...
    for (const prop of allProps) {
      ...
      if (!range.startsWith('http://www.w3.org')) {
        ...
        if (refRange.length !== 0) {
          const containerType = prop.get('?containerType');
          // it's a graph object
          // start recursion
          const isContainer = /list|set/ig.test(containerType ?? '');
          ...
          const subForm = await this._handleClass(
            ...
            isContainer ? '' : `${layoutSubPath}${propName}/properties/`,
          ...
          schema.properties[propName] = isContainer ? {
            type: 'array',
            items: subForm.schema,
          } : subForm.schema;

          layout.elements.push(isContainer ? {
            type: 'Control',
